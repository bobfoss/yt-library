...
- Playlist Newest sorting currently uses the newest member video's upload date so metadata refreshes and owner backfills do not reorder playlists. This is only a content-freshness approximation: adding an old video to a playlist will not make that playlist recent. `playlists.last_changed_at` now retains exact known change evidence or the time a later structural/metadata difference was detected, but it cannot establish when an unseen change actually happened and does not replace per-item chronology. Revisit playlist chronology by capturing a reliable membership-added or first-seen timestamp on `playlist_items`, then decide whether Newest should represent playlist activity, newest content, or offer both sorts.
